...
可選 **`Model`**、**`Version`**：留空則備份時依 `show version` / 等同指令**自動辨識**並重連 NetDriver profile。Cisco 設定備份指令為 `show running-config view full`；Portal 呼叫 Agent 時命令物件使用 **`login`**（不進 `enable`）。

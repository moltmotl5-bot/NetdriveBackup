...
2. **設備總表與版控** — 型號／版本／序號、歷史快照、Running-Config 預覽；Hostname 優先從 running-config 解析；**Cisco Stack / FortiGate HA** 每台實體機一行（Primary/Secondary + 各自 hostname）。FortiGate HA 成員若未正確展開，請重新備份該設備（確保產生 ha_status），再按「重建索引」。，組態仍掛在 Anchor（Active/Primary）上。更新後若出現 Internal Server Error，請重啟服務或按「重建索引」（stack_units 新增 hostname 欄位）。

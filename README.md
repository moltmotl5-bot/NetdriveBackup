...
可選 **`Model`**、**`Version`**：留空則備份時依 `show version` / 等同指令**自動辨識**並重連 NetDriver profile。**Cisco**：`login` 模式、不送 enable；NX-OS 用 `show running-config`，IOS 用 `show running-config view full`。**Huawei／Fortinet**：Agent 使用 `mode: enable`（與 Cisco 的 `login` 欄位分開）。

...
可選 **`Model`**、**`Version`**：留空則備份時依 `show version` / 等同指令**自動辨識**並重連 NetDriver profile。Cisco 全程 **`login`**（NCCM 不送 enable／enable password；Agent Cisco 外掛亦不執行 `enable` CLI）；設定備份為 `show running-config view full`。
...
| Cisco Nexus 備份失敗（auto classify 後） | 若 Detail 為 `[config] …: ^` 但 **CLI tail** 已有 config 與 `hostname#`，為 Agent 誤判 lone `^`（需 **rebuild netdriver-agent**）。否則查權限／`read_timeout`；Cisco 設定備份使用 `show running-config view full`，且 NCCM 以 **`login`** 模式呼叫 Agent（不用 `enable`）。 |

...
def latest_snapshot_dir(device_path: str) -> tuple[str | None, str | None]:
    if not device_path or not os.path.isdir(device_path):
        return None, None
    snap_root = os.path.join(device_path, "snapshots")
    search = snap_root if os.path.isdir(snap_root) else device_path
    dates = sorted(
        [
            d
            for d in os.listdir(search)
            if os.path.isdir(os.path.join(search, d))
        ],
        reverse=True,
    )
    if not dates:
        return None, None
    return dates[0], os.path.join(search, dates[0])
...
def load_device_interface_table(
    ...
    if snapshot:
        snap_root = os.path.join(device_path, "snapshots")
        base = snap_root if os.path.isdir(snap_root) else device_path
        cand = os.path.join(base, snapshot)

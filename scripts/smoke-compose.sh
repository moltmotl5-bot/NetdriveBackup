...
# Agent is not published on the host by default; use docker-compose.dev.yml for local Agent URL checks.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORTAL_URL="${NCCM_SMOKE_PORTAL_URL:-http://127.0.0.1:8501}"
